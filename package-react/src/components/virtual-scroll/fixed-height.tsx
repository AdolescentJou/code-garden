...
const FixedHeight = memo(function ({ list = [], containerHeight = 800, ItemBox = <></>, itemHeight = 50, ...props }: any) {
  ...
  const renderList = useCallback(
    function () {
      const rows = [];
      // 多展示 一屏 limit 个，保证滑动过快不会白屏
      for (let i = startIndex; i <= endIndex + limit; i++) {
        // 渲染每个列表项
        rows.push(
          <ItemBox
            data={i}
