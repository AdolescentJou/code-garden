...
class MyPromise {
  ...
  constructor(handle) {
    ...
    //成功回调函数队列,即then函数中的回调函数
    //注：因为构造函数是异步调用的，所以在构造函数调用成功之前，会先执行then方法，将里面的回调函数存起来
    this._fulfilledQueues = []; 
    //失败回调函数队列,即catch中的回调函数
    this._rejectedQueues = []; 
    //执行handle,构造函数完成后，会回调这里的两个函数
    try {
  ...
  _resolve(val) {
    const run = () => {
      console.log('执行队列中的函数');
      //不是等待状态，直接返回
      if (this._status !== PENDING) return;
      //若状态为FULFILLED，依次执行成功队列中的函数，并清空队列
      ...
      //若状态为REJECTED，依次执行失败队列中的函数，并清空队列
  ...
  then(onFulfilled, onRejected) {
    ...
    return new MyPromise((resolve, reject) => {
      ...
      switch (
        _status // 当状态为pending时，将then方法回调函数加入执行队列等待执行
      ) {
        case PENDING:
...
const test = async () => {
  const res = await new MyPromise((resolve, reject) => {
    ...
    .then((res) => {
      console.log('第 1 个then 函数执行'); // 返回一个Promise对象
      return new MyPromise((resolve, reject) => {
        setTimeout(() => {
    ...
    .then((res) => {
      console.log('第 2 个then 函数执行'); // 返回一个Promise对象
      return new MyPromise((resolve, reject) => {
        setTimeout(() => {
